	...
	#region UI Methods
		...
		private void OnGUI () {
			...
			if (this._IsCreatingNew) {
				...
				GUILayout.Space (3);
				GUILayout.Label ("Create New Pref", EditorStyles.boldLabel);
				this._NewValue.Name = EditorGUILayout.TextField ("New Pref Name : ", this._NewValue.Name);
				...
				if(this._NewValue.Type != valType) {
					Debug.Log("SETTING FROM:" + this._NewValue.Type + " TO:" + valType);
